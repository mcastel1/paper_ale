...
In order to work out the  dynamical equations for \ac{mem}, let us derive the force exerted by \ac{bflu} on \ac{mem}. The force per unit \ac{mem} length $\dint{l_\current}$ is $- \sigmastressc_{\alpha \beta} \normalc{\gamma}$, cf. \cref{eq_fl_rigid_rigid_current_1}.
