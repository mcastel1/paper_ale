...
\begin{figure*}
    ...
    \caption{
        Interaction between a \Acf{bflu}  and a \acf{mem}.
...
Given the \ac{mem} fields $\vmem^{n-1, i}$, $\wmem^{n-1, \, i}$, $\sigmam^{n-3/2}$, $\bumem^{n-3/2}$, $\nu^{n-3/2}$, $\psi^{n-3/2}$, the \ac{dom} fields $\budom^{n-1}$, $\budomdot^{n-1}$, and the \ac{bflu} fields $\bvfr^{n-1}$, $\sigmafr^{n-3/2}$, we determine the fields at the next time step as follows:
...
\begin{itemize}
    \titleditem{Update \ac{mem}}{ \label{fl_mem_step_1}
        ...
        Here, \cref{eq_fl_mem_mem_disc_1,eq_fl_mem_mem_disc_2} are the \acp{pde} for the approximate velocities, \cref{eq_fl_mem_mem_disc_3} for pressure correction, and \cref{eq_fl_mem_mem_disc_4,eq_fl_mem_mem_disc_5} for the velocity correction. \Cref{eq_fl_mem_mem_disc_6} is the time-evolution equation for the displacement field, and \cref{eq_fl_mem_mem_disc_7,eq_fl_mem_mem_disc_8} the discrete versions of \cref{eq-dyn-dX1,eq-dyn-dX2}, respectively.
        ...
        This step determines the \ac{mem} fields $\vmem^{n, i}$, $\wmem^{n, \, i}$, $\sigmam^{n-1/2}$, $\bumem^{n-1/2}$, $\nu^{n-1/2}$, $\psi^{n-1/2}$.
    }
    \titleditem{Update \ac{dom}}{ \label{fl_mem_step_2}
        We determine the \ac{dom} displacement field and its timer derivative, $\budom^n$ and $\budomdot^n$, from the \ac{bvp} \crefs{eq_domain_fluid_rigid_disc,eq_domain_fluid_rigid_disc_dot}, with \acp{bc} given by the discrete version of \cref{bc_fl_mem_dom_1,bc_fl_mem_dom_2,bc_fl_mem_dom_3,bc_fl_mem_dom_4,bc_fl_mem_dom_5,bc_fl_mem_dom_6,bc_fl_mem_dom_7,bc_fl_mem_dom_8}:
        \begin{align}
    ...
    \titleditem{Update \ac{bflu}}{ \label{fl_mem_step_3}
        We determine the \ac{bflu} fields $\vfr^n$ and $\sigmafr^\nmhalf$ by solving the \ac{bvp} \crefs{eq_fl_rigid_fl_1_reference_disc_aux,eq_poiss_like_phi} with \acp{bc}
...
%sign
An example of this dynamics is shown in \cref{fig_fluid_mem}, where we show the interaction between a light gas (\ac{bflu}) and an elastomer (\ac{mem}).  The  channel has dimensions $L = 1 \, \met$, $h = 0.2 \, \met$.  The bending rigidity has been estimated from the relation $\kap = E \delta^3/[12(1-\nu^2)]$, which relates the bending rigidity $\kap$ to the Young modulus $E$, the Poisson ratio $\nu$, and the thickness $\delta$ of  \ac{mem} \cite{landauTheoryElasticity1986}. Typical values for an elastomer   are $E \sim 10^7 \, \pas$ and $\nu \sim 0.5$, where we chose $\delta \sim 10^{-3}\, \met$, yielding $\kap = 10^{-3} \, \kg \, \met^3/\sec^2$.

$\rhom = 1 \, \kg/\met$.

$\etam = 1 \, \kg\, \met /\sec $, and $\sigmaml = 1 \, \newt$.
The \ac{bflu} parameters are $\rhof = 1 \, \kg/\met^2$, $\etaf = 10^{-3} \, \kg / \sec$,  $\sigmafbottomc = - 1 \, \newt / \met^2$, and $\vfbottom^1 =0$, $\vfbottom^2(\bxc) =  \frac{3}{2}  \xc^1 (2 L -\xc^1) v_0$, where $v_0 = 10^{-1}  \, \met / \sec$, and $\vfbottom^2$ corresponds to a Poiseuille flow \cite{landauFluidMechanics1987}.  Finally, the \ac{dom} exponent is $\zeta = 3$ \cite{kamenskyLectureNotesMAE}.
