...
Proceeding along the lines of \cref{sec_fl_rigid}, the  dynamics of the system is obtained as follows.  Given the \ac{mem} fields $v^{n-1, i}$, $w^{n-1, \, i}$, $\sigmam^{n-3/2}$, $\bumem^{n-3/2}$, $\nu^{n-3/2}$, $\psi^{n-3/2}$, the \ac{dom} fields $\budom^n$, $\budom^{n-1}$, and the \ac{bflu} fields $\bvfr^{n-1}$, $\sigmafr^{n-3/2}$, we determine the fields at the next time step as follows.
%sign
...
\begin{itemize}
    \titleditem{Update \ac{mem}}{ \label{fl_mem_step_1}
        ...
        \begin{align}
            ...
            \rhom \left[  \frac{\vbarmem^i - v^{n-1,\, i}}{\deltat} +  \left( \frac{3}{2}v^{n-1,\, j} - \frac{1}{2} v^{n-2,\, j} \right)     \nab^{\nmhalf}_j \Vmem^i - 2 \Vmem^j \Wmem \fnij{b}{\nmhalf}{i}{j} - \Wmem \nab^{\nmhalf, \, i} \Wmem \right] & =              \newlinenn
            G_{\beta i}(\budom^{n-1})  \,          \pderr{ \sigmafr^\ast   }{\beta }                + \etaf \,              G_{\delta \beta}(\budom^{n-1})            \,         \pderr{}{\delta} \left(  G_{\gamma \beta}(\budom^{n-1}) \pderr{\Vmem^i }{\gamma}    \right),
        \end{align}
        \ew
    }
