...
\begin{figure*}
    ...
    \caption{
        Interaction between a \acf{bflu}  and a \acf{mem}. Here, \ac{bflu} is air, and \ac{mem} an elastomer.
...
The  channel has dimensions $L = 1 \, \met$, $h = 0.2 \, \met$.
The \ac{mem} parameters are as follows: The bending rigidity has been estimated from the relation $\kap = E \delta^3/[12(1-\nu^2)]$, which expresses the bending rigidity $\kap$ as a function of the Young modulus $E$, the Poisson ratio $\nu$, and the thickness $\delta$ of  \ac{mem} \cite{landauTheoryElasticity1986}. Typical values for an elastomer   are $E \sim 10^7 \, \pas$ \cite{fengPreparationCharacterizationSilicone2017} and $\nu \sim 0.5$ \cite{mullerQuickAccurateMethod2019}, where we chose $\delta \sim 10^{-3}\, \met$, yielding $\kap = 10^{-3} \, \kg \, \met^3/\sec^2$.
