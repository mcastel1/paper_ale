...
In fact, a general parametrization of \ac{mem} is given by two functions $\bxmemc(\xcoord^1)$, both of which depend on the curvilinear coordinate $\xcoord^1$ which is defined in the \ac{ref} domain $\pomlineeqr$ of \ac{mem}, see \cref{fig_ref_cur_mem}. However, a single \ac{pde}  determines the \ac{mem} shape \cite{zhong-canBendingEnergyVesicle1989}.
...
\begin{align}
    ...
    \nab_i \vmem^i - 2 H w                                                                                                                                                                                                                                                & =                                                                                                 0, \\ \nn
    \label{eq-dyn-v}
    \rhom ( \partial_t v^i + v^j \nab_j v^i  - 2 v^j w b^i_j - w \nab^i w )                                                                                                                                                                                               & =                                                                                                    \\
    \nab^i \sigmam + \etam \left[ - \nablb v^i - 2 \left( b^{ij} - 2 \, H \, g^{ij}  \right) \nab_j w + 2 K v^i   \right] + f_t^i,                                                                                                                                        &                                                                                                      \\ \nn
    \label{eq-dyn-w}
    \rhom \left[ \partial_t w + v^i \left( v^j b_{ji} + \nabla_i w  \right) \right]                                                                                                                                                                                       & =                                                                                                    \\
    - 2\kap \left[   \nab_i \nab^i H  + 2 H (H^2 - K)  \right] + 2 \sigmam H    +                                                         2 \eta \left[ (\nab^i v^j)b_{ij} - 2 w (2 H^2 - K)                                                                 \right]+ f_n & ,
\end{align}
\ew
and they hold for $\xcoord^1 \in \pomlineeqr$. \Cref{eq-dyn-continuity} enforces mass conservation, and \cref{eq-dyn-v,eq-dyn-w} are the tangential and normal components of the \ac{ns} equations, respectively. Also, $v^i$ and $w$ are the \ac{mem} tangential and normal velocity, respectively, $\nab$ is the covariant derivative related to the Levi-Civita connection associated to $g$, $\nablb$ the Laplace-Beltrami operator, $b$ the second fundamental form, and $H$ and $K$  the mean and Gaussian curvature, respectively. We refer to \cite{desernoFluidLipidMembranes2015,worthmullerIRENEFluIdLayeR2025,marchiafavaAppuntiDiGeometria2005} for details on the differential-geometry framework.
