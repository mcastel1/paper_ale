...
The geometry of \cref{fig_ref_cur_mem} may represent \ac{bflu} being confined in a large reservoir, lying below the channel, where the channel is a vertical neck of the reservoir, covered on top by \ac{mem}.

\subsubsection{\Acl{mem} motion}
\label{sec_fl_mem_mem}

Both the physical nature and mathematical description of \ac{mem} is significantly more complex than that of \ac{ela}. First, unlike \ac{ela}, \ac{mem} has a fluid structure: Its material elements can  flow tangentially to the \ac{mem} shape, which also moves subjected to  bending-rigidity forces, and to the force exerted on it by \ac{bflu}.
While the tangential motion is naturally suited to an Eulerian description \cite{landauFluidMechanics1987}, the shape motion---reminescent of that of an elastic body---is best described with a Lagrangian approach \cite{lacarbonaraNonlinearStructuralMechanics2013}.
Also, such shape motion needs to be connected to the Lagrangian approach used to describe \ac{dom}. Second, \ac{mem} is described by fourth-order \acp{pde} \cite{zhong-canBendingEnergyVesicle1989}. From  the mathematical and numerical point of view, such \acp{pde} are significantly more complex than the second-order \acp{pde} which describe \ac{ela} \cite{worthmullerIRENEFluIdLayeR2025}. On top of this, from the physical standpoint, such fourth-order \acp{pde} require more  \acp{bc} than the second-order ones for \ac{ela}: Such \acp{bc} may specify a number of physical features of the \ac{mem} system, such as forces, positions, torques, or stresses.
%sign
Third, the geometrical description of \ac{mem} requires fixing an appropriate \textit{gauge}.
In fact, a general parametrization of \ac{mem} is given by two functions---the two Cartesian components of $\bxmemc(\xcoord^1)$. Both of these functions  depend on the curvilinear coordinate $\xcoord^1$ which is defined in the \ac{ref} domain of \ac{mem}, $\pomlineeqr$, see \cref{fig_ref_cur_mem}---note that here $\xcoord^i$ differs from the Cartesian coordinate $\xcartesian^\alpha$.
...
\end{align}
It is then easy to realize that two  parametrizations with different $\nu$s can yield the same physical shape. Such  redundancy  may be removed by a gauge fixing, i.e., by adding a constraint on $\bxmemc$, or its derivatives.
...
\ee
At the beginning of the dynamics, we choose the \ac{ref} configuration as a straight line
\be
\label{eq_def_x_ref}
\bxmemr(\xcoord^1) \equiv (\xcoord^1, h), \, 0 \leq  \xcoord^1 \leq L,
\ee
see \cref{fig_ref_cur_mem}A.
The mesh will then be deformed with time: As the deformation becomes large, we will remesh the \ac{bflu} domain. The \ac{cur} configuration right before remeshing will then become the \ac{ref} configuration after remeshing, which will be, in general,  different from \crefs{eq_def_x_ref}.
%
...
\begin{align}
    ...
    \label{eq-dyn-v}
    \rhom \left( \pder{\vmem^i}{t}+ \vmem^j \nab_j \vmem^i  - 2 \vmem^j \wmem\, b^i_j - \wmem \nab^i \wmem \right)     = & \fmemt^i + f_{\fluid\, \text{t}}^i,                                                                                                                                                                        \\
    \label{eq-dyn-w}
    \rhom \left[ \pder{\wmem}{t} + \vmem^i ( \vmem^j b_{ji} + \nabla_i \wmem  ) \right]                               =  & 2 \sigmam H    +  f_\kap +                                                         f^\eta_{\mem\, \text{t}}+ f_{\fluid\, \text{n}}                                                                       , \\
    \label{eq-dyn-dX}
    \pder {\bumem}{t}                                                                                            =       & \wmem \normalc{},                                                                                                                                                                                          \\
    \label{eq-dyn-dX1}
    \pder {\umem^1}{x^1}                                                                                            =    & - \pder{\xmemr{1}}{\xcoord^1} + \nu \cos \psi,                                                                                                                                                             \\
    \label{eq-dyn-dX2}
    \pder {\umem^2}{x^1}                                                                                               = & - \pder{\xmemr{2}}{\xcoord^1} -\nu \sin \psi,
\end{align}
\ew
and they hold for $\xcoord^1 \in \omlineeqr$.
%
Here, $\vmem^i$ and $w$ are the \ac{mem} tangential and normal velocity, respectively, $\nab$ is the covariant derivative related to the Levi-Civita connection associated to $g$, and $H$ the mean  curvature.
We refer to \cite{desernoFluidLipidMembranes2015,worthmullerIRENEFluIdLayeR2025,marchiafavaAppuntiDiGeometria2005,arroyoRelaxationDynamicsFluid2009i} for details on the differential-geometry formulation.
% sign
We will now discuss each line in \cref{\eqsmembrane}, and we set
\bw
\begin{align}
    \label{eq_def_f_kap}
    f_\kap (\nu,\psi)                                       & \equiv  -2\kap [ \nab_i \nab^i H + 2 H (H^2-K)],                                  \\
    \label{eq_def_fmemt}
    \fmemt^i(\vmem, \wmem,\sigmam,\nu,\psi)                 & \equiv - \nab_j \Pi^{ij}(\vmem, \wmem,\sigmam,\nu,\psi)                           \\
    \label{eq_def_Pi}
    \Pi^{ij}(\vmem, \wmem,\sigmam,\nu,\psi)                 & \equiv    -   g^{ij}\sigmam - 2 \etam d^{ij}(\vmem, \wmem,\nu,\psi) ,             \\
    \label{eq_def_d}
    d^{ij}(\vmem, \wmem,\nu,\psi)                           & \equiv  \frac{1}{2}\left( \nab^i \vmem^j  + \nab^j \vmem^i\right) - \wmem b^{ij}, \\
    \label{eq_def_f_eta_n}
    f^\eta_{\mem\, \text{t}}      (\vmem, \wmem, \nu, \psi) & \equiv
    2 \etam [ (\nab^i \vmem^j)b_{ij} - 2 \wmem (2 H^2 - K)                                                                 ]  .
\end{align}
\ew
In \cref{eq_def_Pi} $\Pi$, is the momentum-flux density tensor \cite{landauFluidMechanics1987}, which is given by a line-tension term and by a viscous term, where the latter is expressed in terms of the rate-of-deformation tensor $d$ \cite{al-izziShearDrivenInstabilitiesMembrane2020}. Finally, in \cref{eq_def_d}, $b$ is  the second fundamental form \cite{marchiafavaAppuntiDiGeometria2005,desernoNotesDifferentialGeometry2004}.
% 

In \cref{\eqsmembrane}, $\rhom$, $\sigmam$, $\etam$ and $\kappa$ are, respectively, the density, line tension, viscosity, and bending rigidity of \ac{mem}.
...
The term $\fmemt$ in \cref{eq-dyn-v} represents the force on \ac{mem} elements induced by gradients of line tension and viscous effects, cf. \cref{eq_def_fmemt}, while $f_{\fluid\, \text{t}}$ is the force exerted on \ac{mem} elements by \ac{bflu}.
%
The first term in the \ac{rhs} of \cref{eq-dyn-w} constitutes Laplace pressure \cite{degennesCapillarityWettingPhenomena2004,landauFluidMechanics1987},  $f_\kappa$ represents the resistance of \ac{mem} to bending \cite{helfrichElasticPropertiesLipid1973}, cf. \cref{eq_def_f_kap},  while $f^\eta_{\mem\, \text{t}}$ and $f_{\fluid\, \text{n}}$ are  the  normal components of the viscous force resulting from in-membrane flows \cite{arroyoRelaxationDynamicsFluid2009i} and of the force exerted by \ac{bflu} on \ac{mem}, respectively.
% sign
%
\Cref{eq-dyn-dX} is the time-evolution equation for the \ac{mem} shape, which moves according to the normal $\normalc{}$, and it has been obtained from  \cref{eq_u_mem}.
We observe that the \ac{mem} shape velocity, $\partial \bumem / \partial t$, coincides with the normal displacement velocity of \ac{mem}, but  it is independent of the tangential flow velocity $\bvmem$ of \ac{mem} elements along the shape. This implies that the \ac{mem} shape and \ac{bflu} motions are geometrically conforming \cite{kamenskyLectureNotesMAE}, but that the tangential part of the \ac{bflu} velocity at \ac{mem} need not coincide with $\bvmem$, thus allowing for a slip \cite{landauFluidMechanics1987}.
%
Finally, \cref{eq-dyn-dX1,eq-dyn-dX2} re-express the definitions of $\nu$ and $\psi$, and  they result from \cref{def_nu_psi_1,def_nu_psi_2,eq_def_e,eq_u_mem}.



We choose the following \acp{bc} for \cref{\eqsmembrane}:
\begin{align}
    \label{bc_fl_mem_mem_1}
    \normalmem_i \Pi^{ij} = 0 & = 0 \text{ on } \pomlineineqr,        \\
...
\ee
where $F$ is defined in \cref{eq_def_F}, and
\begin{align}
    \label{eq_ela_1}
...
\end{align}
\Cref{bc_fl_mem_dom_1} ensures that \ac{dom} is not deformed on $\pomineqr \cup \pombottomeqr$, cf. \cref{bc_ela_1}. \Cref{bc_fl_mem_dom_2,bc_fl_mem_dom_3} follow from axial symmetry, and \cref{bc_fl_mem_dom_4} from the requirement that \ac{dom} and \ac{mem} deformations stay conforming \cite{kamenskyLectureNotesMAE}. Finally, the  \acp{bc} for $\budomdot$, \cref{bc_fl_mem_dom_5,bc_fl_mem_dom_6,bc_fl_mem_dom_7,bc_fl_mem_dom_8}, are derived along the same lines as \cref{bc_fl_mem_dom_1,bc_fl_mem_dom_2,bc_fl_mem_dom_3,bc_fl_mem_dom_4}.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth,height=0.55\textheight,keepaspectratio]{figures/figure_9/figure_9.pdf}
    \caption{
...
\begin{figure*}
    ...
    \caption{
        Interaction between a \acf{bflu}  and a \ac{mem}: \ac{mem} fields. Here, \ac{bflu} is air, and \ac{mem} an elastomer, and the \ac{bflu} Reynolds number is $R \sim 200$.
        \plab{A}  Initial and \acf{cur} configuration of \ac{mem} (solid blue and dashed red curve, respectively).
        \plab{B} Stretch field of \ac{mem}.
        \plab{C} Tangent angle of \ac{mem}.
        \plab{D}, \plab{E} and \plab{F}: \ac{mem} angential velocity, normal velocity and tension, respectively.
        All panels refer to the same instant of time, shown on top, and they display also the mesh.
        \label{fig_fluid_mem_mem}
    }
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/figure_23/figure_23.pdf}
    \caption{
        Interaction between a \acf{bflu}  and a \ac{mem}: \ac{bflu} fields. The solution and notation is the same as  \cref{fig_fluid_mem_mem}.
        \plab{A} and \plab{B}: \ac{bflu} velocity and surface tension, respectively.
        \label{fig_fluid_mem_bflu}
    }
\end{figure*}
...
%
\Cref{bc_fl_membrane_5_current} ensures that the velocity of \ac{bflu} and of the \ac{mem} shape coincide, allowing for a slip between the tangential flow of \ac{mem}  elements and \ac{bflu} material elements, see \cref{sec_fl_mem_mem} and \cref{eq-dyn-dX} in there.
%
...
\begin{align}
    ...
    - \nab^\nmhalf_j \Pi^{ij} \left( \frac{\bvmem^n + \bvmem^{n-1}}{2}, \frac{\wmem^n + \wmem^{n-1}}{2},\sigmam^\nmhalf,\nu^\nmhalf,\psi^\nmhalf \right)+ \newlinenn
    ...
    f_\kap(\nu^\nmhalf,\psi^\nmhalf) + 2 \sigmam^\nmhalf H^\nmhalf + f^\eta_{\mem\, \text{t}}     \left( \frac{\bvmem^n + \bvmem^{n-1}}{2}, \frac{\wmem^n + \wmem^{n-1}}{2}, \nu^\nmhalf, \psi^\nmhalf\right) + \newlinenn
...
\begin{enumerate}[label=\currentsec.\arabic*]
    \titleditem{Approximated velocities}{
        ...
        \begin{align}
            ...
            - \nab^\nmhalf_j \Pi^{ij}(\Vmem, \Wmem,\sigmamast,\nu^\nmhalf,\psi^\nmhalf) + \newlinenn
            ...
            f_\kap(\nu^\nmhalf,\psi^\nmhalf) + 2 \sigmamast H^\nmhalf + f^\eta_{\mem\, \text{t}}      (\bVmem, \Wmem, \nu^\nmhalf, \psi^\nmhalf) + \newlinenn
...
An example of this dynamics is shown in \cref{fig_fluid_mem_mem}, where we show the interaction between air (\ac{bflu}) and an elastomer (\ac{mem}).
The  channel has dimensions $L = 1 \, \met$, $h = 0.2 \, \met$. Proceeding along the lines of \cref{sec_fl_rigid},   \ac{mem} parameters have been estimated by considering the parameters for a three-dimensional material, multiplying them by a unit length corresponding to the out-of-plane depth, and by the \ac{mem} thickness, chosen to be $\Delta = 10^{-3}\, \met$. The \ac{mem} bending rigidity $\kap$ has been estimated from the relation $\kap = E \Delta^3/[12(1-\poissonratio^2)]$, which expresses $\kap$ as a function of the Young modulus $E$, the Poisson ratio $\poissonratio$, and  $\Delta$ \cite{landauTheoryElasticity1986}. Typical values for an elastomer   are $E \sim 10^7 \, \pas$ \cite{fengPreparationCharacterizationSilicone2017} and $\poissonratio \sim 0.5$ \cite{mullerQuickAccurateMethod2019}, yielding $\kap = 10^{-3} \, \kg \, \met^3/\sec^2$.
...
\begin{figure}
    ...
    \caption{
        \label{fig_no_circle}
        Curvature of the \Acl{mem}  interacting  with a \acl{bflu} shown in \cref{fig_fluid_mem_mem}. The Figure follows the same notation and it corresponds to the same instant of time as \cref{fig_fluid_mem_mem}.
    }
\end{figure}