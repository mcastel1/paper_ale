...
We will now discuss each line in \cref{\eqsmembrane}.
% 
...
\end{align}
\ew
In particular, in \cref{eq_def_Pi} $\Pi$ is the stress tensor \cite{worthmullerIRENEFluIdLayeR2025}, which is given by a line-tension term and by a viscous term, where the latter contains the rate-of-deformation tensor $d$ \cite{al-izziShearDrivenInstabilitiesMembrane2020}. Finally, in \cref{eq_def_d}, $b$ the second fundamental form \cite{marchiafavaAppuntiDiGeometria2005,desernoNotesDifferentialGeometry2004}.
% 
...
%
The first term in the \ac{rhs} of \cref{eq-dyn-w} constitutes Laplace pressure \cite{degennesCapillarityWettingPhenomena2004,landauFluidMechanics1987},  $f_\kappa$ represents the resistance of \ac{mem} to bending \cite{helfrichElasticPropertiesLipid1973}, cf. \cref{eq_def_f_kap},  while $f_{\etam\, \text{n}}$ and $f_{\fluid\, \text{n}}$ are  the  normal components of the viscous force resulting from in-membrane flows \cite{arroyoRelaxationDynamicsFluid2009i} and of the force exerted by \ac{bflu} on \ac{mem}, respectively.
%
