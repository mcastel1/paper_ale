...
\begin{align}
    \label{def_force_tan}
    f_{\fluid\, t}^i(\sigmastressc, \nu, \psi) & = g^{ij}(\nu) \evec{j}{\alpha}(\nu,\psi) f_\alpha(\sigmastressc, \nu, \psi) \\
    \label{def_force_norm}
    f_{\fluid\, n} (\sigmastressc, \nu, \psi)  & = \normalc{\alpha}(\nu, \psi) f_\alpha(\sigmastressc, \nu, \psi),
\end{align}
...
The equation of motion for $\budomdot$ are obtained by deriving \cref{eq_mesh_fluid_rigid} with respect to time, and using the fact that \ac{ref} coordinates are time independent:
\be
...
The \ac{bflu} equations of motion are
% 
\begin{align}
...
\Cref{eq_fl_rigid_fl_1_current,eq_fl_rigid_fl_2_current} are the \ac{ns} and continuity equations, respectively, in a flat, two-dimensional geometry \cite{landauFluidMechanics1987}---an example of the corresponding  \ac{ns} equations on curved geometry is given by \cref{eq-dyn-v}.
% 
...
\Cref{bc_fl_membrane_5_current} ensures that the motion of \ac{bflu} is conforming to that of \ac{mem}   \cite{kamenskyLectureNotesMAE}: According to  \cref{eq-dyn-dX}, \ac{mem}  only moves normally to its profile; \cref{bc_fl_membrane_5_current} thus ensures that \ac{bflu} stays in contact with \ac{mem} throughout its motion. On the other hand, \cref{bc_fl_membrane_5_current} implies a tangential slip between \ac{mem} and \ac{bflu}, allowing for the tangential part of the \ac{bflu} and \ac{mem} velocities to differ.
Finally,  \cref{bc_fl_membrane_6_current} stems from the presence of the reservoir, in which the \ac{bflu}  tension is equal to $\sigmafbottomc$: At the boundary $\pombottomeqc$ between the channel and the reservoir, the \ac{bflu} tension  matches the reservoir's.\vspace{0.5cm}\\
