...
The first term in the \ac{rhs} of \cref{eq-dyn-v} represents the force on membrane elements induced by gradients of line tension, while the first term in the \ac{rhs} of \cref{eq-dyn-w} constitutes Laplace pressure \cite{degennesCapillarityWettingPhenomena2004,landauFluidMechanics1987}. The other terms read
\bw
\begin{align}
        \label{eq_def_f_kap}
        f_\kap (\nu,\psi)                                   & \equiv  -2\kap [ \nab_i \nab^i H + 2 H (H^2-K)],                                                             \\
...
\end{align}
\ew
In \cref{eq_def_f_kap}, $f_\kappa$ is represents the resistance of \ac{mem} to bending, and it is derived from Helfrich model \cite{helfrichElasticPropertiesLipid1973}, while $f_{\etam\, t}$ and $f_{\etam\, \text{n}}$ in \cref{eq_def_f_eta_t,eq_def_f_eta_n} are, respectively, the tangential and normal component of the viscous force resulting from in-membrane flows \cite{arroyoRelaxationDynamicsFluid2009i}. In particular, in \cref{eq_def_f_eta_t}, $d$ is the rate-of-deformation tensor \cite{al-izziShearDrivenInstabilitiesMembrane2020}.
...
We observe that the \ac{mem} shape velocity, $\pder{\bumem}{t}$, coincides with the normal displacement velocity of \ac{mem}, but is it independent of the tangential flow velocity $\bvmem$ of \ac{mem} elements along the shape. This implies that the \ac{mem} shape and \ac{bflu} motions are conforming \cite{kamenskyLectureNotesMAE}, but that the tangential part of the \ac{bflu} velocity at \ac{mem} need not coincide with $\bvmem$, thus allowing for a slip \cite{landauFluidMechanics1987}.
%
Finally, \cref{eq-dyn-dX1,eq-dyn-dX2} re-express the definitions of $\nu$ and $\psi$, and  result from \cref{def_nu_psi_1,def_nu_psi_2,eq_def_e,eq_u_mem,eq_def_x_ref}, and
$\rhom$, $\sigmam$, $\etam$ and $\kappa$ are, respectively, the density, line tension, viscosity, and bending rigidity of \ac{mem}.
