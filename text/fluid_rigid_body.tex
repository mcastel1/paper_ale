...
\begin{align}
        ...
        \sigmastressr^t_{\alpha \beta}(\bxr)  \equiv & \sigmastressc^t_{\alpha \beta}({\bm \phi}^t(\bxr)) \newlinenn
...
\begin{align}
        ...
        \pder{}{\xr^\beta}\der{S_{\alpha \beta}}{t} & = 0 \text{ in } \omr,                                                             \\
        \label{bc_ela_1_dot}
        \budot                                      & =\bzero \text{ on } \pomsqeqr,                                                    \\
        \label{bc_ela_2_dot}
        \budot                                      & =  \omega \der{{\bm R}}{\theta}\cdot (\bxr - {\bm c}) \text{ on } \pomellipseeqr,
\end{align}
where in \cref{bc_ela_1_dot,bc_ela_2_dot} we used the fact that $\bxr$ is independent of time, and in \cref{bc_ela_2_dot} we substituted \cref{eq_fl_rigid_rigid_current_2}. The \ac{lhs} of \cref{eq_mesh_fluid_rigid_dot} depends on the second partial derivatives of $\bu$ and $\budot$. As a result, given $\theta$, $\omega$, and $\bu$ from \cref{eq_mesh_fluid_rigid,bc_ela_1,bc_ela_2}, the \ac{bvp} \crefs{eq_mesh_fluid_rigid_dot,bc_ela_1_dot,bc_ela_2_dot}  yields $\budot$.\vspace{0.5cm}\\
...
To discretize in time the \ac{bflu} problem, we will apply the \ac{cn} splitting scheme \cite{loggAutomatedSolutionDifferential2012,crankPracticalMethodNumerical1947} to the \ac{ns} equations \crefs{eq_fl_rigid_fl_1_reference,eq_fl_rigid_fl_2_reference} in the \ac{ref} configurations.
...
\begin{align}
        ...
        {\bm \vfr}                                                                                                    & =
        \omega^n  \left. \der{{\bm R}}{\theta} \right|_{\theta = \theta_n} \cdot (\bxr - {\bm c})  \text{ on } \pomellipseeqr ,                                                                                                                                   \\
...
The discrete version of  the \ac{bvp} \crefs{eq_mesh_fluid_rigid,bc_ela_1,bc_ela_2} is
\begin{align}
