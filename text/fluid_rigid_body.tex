...
\begin{align}
        ...
        I \der{\omega}{t} & =                                               \newlinenn
        \int_\pomellipseeqr \dint{s} \, \epsilon_{\alpha \beta} \left[ \phi^{\alpha, \, t}(\bxi{s}) - \focalpoint^\alpha \right] \sigmastressr_{\beta \gamma}(\bxi{s}) \epsilon_{\gamma \lambda} F^t_{\lambda \mu}(\bxi{s}) \der{\xi^\mu(s)}{s}
\end{align}
\ew
...
We will now split the \ac{bvp} into three steps
\begin{enumerate}
        \item \label{item_split_1}
              Introduce the velocity $\bvbar$, which approximates the true velocity $\bm \vfr$, and which  satisfies the \ac{bvp}
              \bw
              \begin{align}
