...
An example of such dynamics is shown in \cref{fig_fluid_rigid}, where we study the interaction between air (\ac{bflu}) and an aerogel (\ac{bod}). The channel has dimensions $L = 2.2 \, \met$, $h =  0.41 \, \met$, which have been taken from the FEAT2D DFG 2D-3 benchmark for a flow around a cylinder
%M add original reference of  FEAT2D DFG 2D-3 
\cite{blumFEAT2DFiniteElement1995}, see  \cref{fig_ref_curr_rigid}.
The parameters for \ac{bod} have been estimated from the  properties of air:
%M add citation
$\rhof = 1 \, \kg / \met^2
    %M add citation
    , \etaf = 10^{-5} \, \pas\, \met \, \sec$. The  inflow velocity profile is chosen as a Poiseuille flow  $\vfin(\bxc) = 6 v_0 \xc^2 (\xc^2 - h)/h^2 $, with $v_0 = 1 \, \met/\sec$. The \ac{dom} mesh stiffness exponent  has been chosen to be $\zeta = 3$ \cite{kamenskyLectureNotesMAE}.  Finally,  \ac{rig} is an ellipse with semi-axes $a = 0.1\, \met$, $b = 0.075\, \met$,  center located at  $\bxr = (0.25 \, \met, 0.2\, \met)$, and moment of inertia $I =10^{-3} \,  \kg \, \met^2$. These parameters  correspond approximately to a light gas interacting with a solid body made of light wood.\\
...
\ee
For the parameters of \cref{fig_fluid_rigid}, \cref{eq_expressions_tau_short} yields $\tbody \sim 13 \, \sec$, which is in agreement with the  period of the oscillations in \cref{fig_fluid_rigid_timescale}.
