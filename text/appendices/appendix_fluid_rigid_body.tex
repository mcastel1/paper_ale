...
Here $\sigmafc$ is the \ac{bflu} surface tension, and $\sigmastressc_{\alpha \beta }$ the \ac{bflu} stress tensor in  \ac{cur} coordinates:
\be
...
\ee
where $\epsilon_{\alpha \beta \gamma}$ is the three-dimensional Levi-Civita symbol, and $\etaf$ $\rhof$ the two-dimensional viscosity and density of \ac{bflu}, respectively.
\Cref{eq_fl_rigid_fl_1_current,eq_fl_rigid_fl_2_current} are the \ac{ns} and continuity equation, respectively, in a flat, two-dimensional geometry \cite{landauFluidMechanics1987}---an example of the \ac{ns} equations on curved geometry will be presented in X.
