...
\begin{align}
    \label{eq_vf_ref}
    \bvfc(\bxr, t)    & \equiv {\bm \vfr}( \bm{\varphi}^t(\bxr), t),     \\
    \label{eq_sigmaf_ref}
    \sigmafc(\bxr, t) & \equiv {\bm \sigmafr}( \bm{\varphi}^t(\bxr), t).
\end{align}


We will now rewrite the equations of motion and the \acp{bc}   in terms of $\bvfc$, $\sigmafc$, and the \ac{ref} coordinate $\bxr$. The equations of motion \crefs{eq_fl_rigid_fl_1_current,eq_fl_rigid_fl_2_current} yield
\bw
\begin{align}
...
We will thus determine $\budom$ as the solution of a \ac{bvp} given by the following elastic model \cite{kamenskyLectureNotesMAE,marsdenMathematicalFoundationsElasticity1994}
...
\begin{align}
    \label{eq_ela_1}
    T_{\alpha \beta} & \equiv \kel(\budom) E_{\gamma \gamma} \delta_{\alpha \beta} +2 \muel(\budom) \left[E_{\alpha \beta} - \frac{1}{2} \delta_{\alpha \beta} E_{\gamma \gamma}\right], \\
    \label{eq_ela_2}
...
From  the \ac{bvp} above, we obtain a \ac{bvp} for $\dot{\budom}$, which will be needed in the following to describe the \ac{dom} motion. By deriving both sides of \cref{eq_mesh_fluid_rigid,bc_ela_1,bc_ela_2}, we obtain, respectively,
\begin{align}
...
\end{align}
where in \cref{bc_ela_1_dot,bc_ela_2_dot} we used the fact that $\bxr$ is independent of time, and in \cref{bc_ela_2_dot} we substituted \cref{eq_fl_rigid_rigid_current_2}. The \ac{lhs} of \cref{eq_mesh_fluid_rigid_dot} depends on the second partial derivatives of $\budom$ and $\budomdot$---see \cref{eq_def_S,eq_ela_1,eq_ela_2,eq_ela_3,eq_ela_4}. As a result, given $\theta$, $\omega$, and $\budom$ from \cref{eq_mesh_fluid_rigid,bc_ela_1,bc_ela_2}, the \ac{bvp} \crefs{eq_mesh_fluid_rigid_dot,bc_ela_1_dot,bc_ela_2_dot}  yields $\budomdot$.\vspace{0.5cm}\\
...
\begin{enumerate}
    \titleditem{Approximated velocity}{
        \label{item_split_1}
        We introduce the velocity $\bvbarf$, which approximates the true velocity $\bm \vfr$, and which  satisfies the \ac{bvp}
        \bw
        \begin{align}
            \label{eq_fl_rigid_fl_1_reference_disc_aux}
            \rhof \left\{  \frac{\vbarf^\alpha - \vfr^{n-1,\, \alpha}}{\deltat} + \left[ \frac{3}{2} ( \vfr^{n-1,\, \gamma}  -  \udomdot^{n-1,\, \gamma}) \, G_{\beta \gamma}(\budom^{n-1})  - \frac{1}{2} ( \vfr^{n-2,\, \gamma}  -  \udomdot^{n-2,\, \gamma}) \, G_{\beta \gamma}(\budom^{n-2})  \right] \frac{\partial  V^\alpha}{\partial \xr^\beta}\right\} & =              \newlinenn
            G_{\beta \alpha}(\budom^{n-1})  \,          \pderr{ \sigmafr^\ast   }{\beta }                + \etaf \,              G_{\delta \beta}(\budom^{n-1})            \,         \pderr{}{\delta} \left(  G_{\gamma \beta}(\budom^{n-1}) \pderr{V^\alpha }{\gamma}    \right),
        \end{align}
        \begin{align}
            \label{bc_fl_rigid_1_reference_disc_aux}
            \vbarf^1                                                   & = v_\ast \; \text{ on }\pomineqr,                                                                                                        \\
            ...
            \etaf \, G_{\beta 1}(\budom^{n-1}) \pderr{V^\alpha}{\beta} & = 0                                                                                                           \; \text{ on } \pomouteqr.
        \end{align}
        \ew
        where
        \begin{align}
            {\bm V}       & \equiv\frac{ \bvbarf + {\bm \vfr}^{n-1}}{2}, \\
    ...
    \titleditem{Pressure correction}{
        ...
        \be
        \label{eq_phi_1}
        \rhof \frac{\vbarf^\alpha - \vfr^{n, \, \alpha}}{\deltat}  = G_{\beta \alpha}(\budom^{n-1})         \pderr{ \phi   }{\beta } \text{ in } \omr,
        \ee
        where
        \be\label{eq_def_phi_pressure}
        \phi \equiv \sigmafc^\ast - \sigmafc^{n-1/2}
        \ee
        is the surface-tension increment.                 By applying $G_{\gamma \alpha}(\budom^{n-1}) \partial / \partial \xr^\gamma$ to both sides of \cref{eq_phi_1} and using \cref{eq_fl_rigid_fl_2_reference_disc}, we obtain a second-order \ac{pde} for $\phi$:
        \begin{align}
            \label{eq_poiss_like_phi}
            \frac{\rhof}{\deltat} G_{\gamma \alpha}(\budom^{n-1}) \pderr{\vbarf^\alpha }{\gamma}                                                & = \newlinenn
            G_{\gamma \alpha}(\budom^{n-1}) \pderr{}{\gamma}\left( G_{\beta \alpha}(\budom^{n-1})  \,          \pderr{ \phi   }{\beta } \right) & .
        \end{align}
        ...
        \begin{align}
            ...
            \normalr{\gamma} G_{\gamma \alpha}(\budom^{n-1})  G_{\beta \alpha}(\budom^{n-1})  \,          \pderr{ \phi   }{\beta }.
        \end{align}
        ...
        \end{align}
        The \ac{bvp} \crefs{eq_poiss_like_phi,bc_phi_1,bc_phi_2} determines $\phi$ and, through the definition \crefs{eq_def_phi_pressure}, the surface tension $\sigmac^{n-1/2}$.
    }
    \titleditem{Velocity correction}{
        \label{item_split_3}
        The velocity ${\bm \vfr}^n$ is determined from the approximate velocity $\bvbarf$ from \cref{eq_phi_1}.
...
The discrete version of  the \ac{bvp} for $\budom$, \cref{eq_mesh_fluid_rigid,bc_ela_1,bc_ela_2}, reads
\begin{align}
...
\end{align}
and that of the \ac{bvp} for $\budomdot$, \cref{eq_mesh_fluid_rigid_dot,bc_ela_1_dot,bc_ela_2_dot}, is
\begin{align}
    \label{eq_domain_fluid_rigid_disc_dot}
    \pder{}{\xr^\beta}\der{\Sdom_{\alpha \beta}(\budomdot^n)}{t} & = 0 \text{ in } \omr,                                                                  \\
...
An example of such dynamics is shown in \cref{fig_fluid_rigid}. The channel has dimensions $L = 2.2 \, \met$, $h =  0.41 \, \met$, cf. \cref{fig_ref_curr_rigid}, $\rhof = 1 \, \kg / \met^2, \etaf = 10^{-3} \, \pas\, \met \, \sec$, and the  inflow velocity profile is a Poiseuille flow  $v_\ast(\bxc) = 6 v_0 \xc^2 (\xc^2 - h)/h^2 $, with $v_0 = 1 \, \met/\sec$. These parameters have been taken from the FEAT2D DFG 2D-3 benchmark for a flow around a cylinder \cite{blumFEAT2DFiniteElement1995}. The \ac{dom} mesh stiffness exponent  has been chosen to be $\zeta = 3$ \cite{kamenskyLectureNotesMAE}.  Finally,  \ac{rig} is an ellipse with semi-axes $a = 0.1\, \met$, $b = 0.075\, \met$,  center located at  $\bxr = (0.25 \, \met, 0.2\, \met)$, and moment of inertia $I =10^{-1} \,  \kg \, \met^2$. These parameters  correspond approximately to a light gas interacting with a solid body made of light wood.\\
...
\begin{figure*}
    ...
    \caption{
        ...
        Interaction between a  \acl{bflu} and a rigid \acl{bod}, whose material properties roughly correspond to a light gas and a body made of light wood, respectively. \plab{A} Temporal snapshot of the mesh (black lines) and  velocity field (arrows) at the \acl{cur} time $t$, shown on top. The direction of the velocity field is indicated by the arrows, and its norm by their color; the direction of arrows with velocity close to zero is not defined. The  \acl{bod} (ellipse) is allowed to pivot about its left focal point (red dot), and the related angle with respect to the $\xc^1$ direction is denoted by a red arc.   \plab{B} Color map of the surface tension at the \acl{cur} instant of time $t$.
    }
\end{figure*}
...
\begin{figure}
    ...
    \caption{
        ...
        Angle of the \acl{bod} as a function of time for the interaction between a  \acl{bflu} and a rigid \acl{bod} shown in \cref{fig_fluid_rigid}.
    }
\end{figure}

As shown in \cref{fig_fluid_rigid_timescale}, the dynamics of \cref{fig_fluid_rigid} involves two physical time scales: A short one, $\tshort$, related to fast angular oscillations of \ac{bod}, and a long one,  $\tlong$, given by the damping of such oscillations.

First, the scale $\tshort$ can be estimated by considering the dynamical equation \crefs{eq_fl_rigid_rigid_current_1} for \ac{bod}, and approximating its \ac{lhs} as $I / \tshort^2$ and its \ac{rhs} as $ (\etaf v_0 /a \, 2 \pi a) a$. Here,  $\etaf v_0 /a$ is the viscous component of the force per unit length exerted by \ac{bflu} on \ac{bod}, cf. \cref{eq_strees_tensor_fluid}, $2 \pi a$ estimates the total length of the boundary of \ac{bod}, and $a$ approximates the lever arm of the force above. Putting everything together, we obtain
\be
\label{eq_expressions_tau_short}
\tshort \sim \sqrt{\frac{I}{2 \pi a \etaf v_0 }}.
\ee
For the parameters of \cref{fig_fluid_rigid}, \cref{eq_expressions_tau_short} yields $\tshort \sim 13 \, \sec$, which is in agreement with the  period of the oscillations in \cref{fig_fluid_rigid_timescale}.

Second, $\tlong$  can be estimated by building a combination of the material parameters $\rhof$, $\etaf$ and $I$ of the model. The only parameter combination that can yield a quantity with the dimensions of a time is
\be
\label{eq_def_tau_long}
\tlong \sim \frac{\sqrt{\rhof I }}{\etaf}.
\ee
For the parameters of \cref{fig_fluid_rigid}, \cref{eq_def_tau_long} yields $\tlong \sim 316 \, \sec$, which is roughly the timescale at which the width of the oscillations in \cref{fig_fluid_rigid_timescale} is dampened out.