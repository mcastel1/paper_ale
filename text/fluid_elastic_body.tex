...
The channel dimensions and \ac{bflu} parameters have been taken from the FEAT2D DFG 2D-3 benchmark example for a fluid flow around a cylinder \cite{blumFEAT2DFiniteElement1995}.
The \ac{ela} density and modulus of compression are, respectively, $\rhoela = 10^3 \,\kg/\met^2$ \cite{xuConcurrentStiffeningSoftening2023} and a storage modulus of $\sim 10 \, \pas$ \cite{lichtSynthetic3DPEGAnisogel2019}.
Given a \ac{ela} Poisson ratio \cite{landauTheoryElasticity1986} of $\nu \sim 0.5$ \cite{chippadaSimultaneousDeterminationYoungs2010}.
% revise - end
% and $\kel = \muel = 10\,  \kg/\sec^2$. The other parameters are the same as in \cref{fig_fluid_rigid}.
